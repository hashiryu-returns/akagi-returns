...
# into <repo-root>/runtime/{python,uv}/<triple>/. `try_bundled` in
# src/bot/runtime.rs picks that tree up exe-adjacent at runtime, which
# scripts/run-local.sh arranges by linking runtime/ into target/debug/.
#
# Usage:
