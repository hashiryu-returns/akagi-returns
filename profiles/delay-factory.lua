-- Profile: factory — Akagi's shipped delay model, unmodified.
--
...
-- conditional bonuses further down were layered on top of clusters
-- already fitted to the marginal distribution, so the same think time
-- gets paid for twice. Its output distribution sits closer to Bronze-room
-- opponents than to the higher-room players it claims to model. See
-- majsoul-wire-timing docs/03-fitting-a-delay-model.md and the header of delay-calibrated.lua.
--
-- It also has no model for the dealer's opening discard: that decision comes
-- out at a ~3.0 s median, under the free window, so the budget shaping below
-- never unlocks the bank and the soft cap flattens nearly every opening onto
-- one value. Measured on the wire that was 62% of our openings in a single
-- whole-second bucket against a human 15%. Only delay-calibrated.lua fixes
-- it. Keep that in mind when falling back here.
--
